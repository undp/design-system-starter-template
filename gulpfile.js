...
// const $ = plugins({ pattern: ['gulp-if', 'gulp-sourcemaps', 'gulp-clean-css', 'gulp-terser']});
...
gulp.task('default',
  gulp.series('process', server, watch)
);

gulp.task('build', gulp.series(setProd, 'process'));
...
function javascript() {
  return gulp.src(PATH_JS)
    ...
    .pipe(gulpif(PRODUCTION, gulpTerser({}, minify)
